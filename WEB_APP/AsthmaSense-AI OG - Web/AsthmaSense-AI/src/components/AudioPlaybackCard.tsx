...
export default function AudioPlaybackCard({
  ...
  // Resolve audio track from prop, memory cache, or IndexedDB storage
  useEffect(() => {
      ...
      if (Platform.OS === 'web') {
        if (resolvedUri) {
          if (!audioRef.current || audioRef.current.src !== resolvedUri) {
            audioRef.current = new window.Audio(resolvedUri);
          ...
          // Acoustic Tone synthesis fallback
          ...
          if (AudioContextClass) {
            ...
            osc.type = isWheeze ? 'sawtooth' : 'sine';
            osc.frequency.setValueAtTime(isWheeze ? 520 : 180, ctx.currentTime);
  ...
  const handleSeek = (event: any) => {
    ...
    try {
      ...
      // Approximate track width is ~200px
