    ...
    if (Platform.OS === 'web' && uri && uri.startsWith('blob:')) {
      try {
        ...
        uri = await new Promise<string>((resolve) => {
          ...
          reader.onerror = () => resolve(audioRecorder.uri!);
          reader.readAsDataURL(blob);
        });
      } catch (_) {}
    }

    const fileName = `recording_${Date.now().toString().slice(-6)}.m4a`;
    if (typeof window !== 'undefined') {
      (window as any).__asthma_audio_cache = (window as any).__asthma_audio_cache || {};
      (window as any).__asthma_audio_cache[fileName] = uri;
    }

    setFile(
      uri
        ? { name: fileName, uri, sizeKb: 148 }
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        try {
          ...
          console.warn('Failed to read file as data URL, using asset.uri:', readErr);
        }
      }

      const fileName = asset.name || 'uploaded_audio.m4a';
      if (typeof window !== 'undefined') {
        (window as any).__asthma_audio_cache = (window as any).__asthma_audio_cache || {};
        (window as any).__asthma_audio_cache[fileName] = persistentUri;
      }
  ...
  const triggerAnalysis = async () => {
    ...
    try {
      ...
      if (apiHealth.ok && file?.uri) {
        ...
        // Convert audio to persistent data URL for offline replay in reports
        let persistentAudioUri = file?.uri;
        if (Platform.OS === 'web' && file?.uri && file.uri.startsWith('blob:')) {
          try {
            const res = await fetch(file.uri);
            const blob = await res.blob();
            persistentAudioUri = await new Promise<string>((resolve) => {
              const reader = new FileReader();
              reader.onloadend = () => resolve(reader.result as string);
              reader.onerror = () => resolve(file.uri!);
              reader.readAsDataURL(blob);
            });
          } catch (e) {
            console.warn('Failed to convert blob to persistent data URL:', e);
          }
        }
        ...
        setTimeout(() => {
          ...
          // Auto-persist report with audio recording
          addReport({
            audioFileId: (result as any).audio?.audioId,
            audioUri: (result as any).audio?.dataUri || persistentAudioUri,
            audioUrl: (result as any).audio?.url,
            fileName: (result as any).audio?.fileName || file?.name || 'respiratory_audio.wav',
            audioMimeType: (result as any).audio?.mimeType || 'audio/wav',
            audioDuration: (result as any).audio?.duration || (time > 0 ? time : 5),
            audioSize: (result as any).audio?.size || 0,
            wheezingDetected: resolvedResult.wheezingDetected,
            riskLevel: resolvedResult.riskLevel,
            confidence: resolvedResult.confidence,
            summary: resolvedResult.summary,
            clinicalFindings: resolvedResult.clinicalFindings,
            transcript: resolvedResult.transcript,
            rr: resolvedResult.rr || '16 bpm',
            pattern: resolvedResult.pattern || 'Clear · Regular',
            regularity: resolvedResult.regularity || '92%',
            wheezePattern: resolvedResult.wheezePattern || (resolvedResult.wheezingDetected === 'Yes' ? 'Audible whistle' : 'None detected'),
            recommendedExercise: resolvedResult.recommendedExercise,
            recommendations: resolvedResult.recommendations,
            foodsToEat: resolvedResult.foodsToEat,
            foodsToAvoid: resolvedResult.foodsToAvoid,
          });
          setReportSaved(true);
        }, 3600);
      } else {
      ...
      setTimeout(() => {
        ...
        setAlertConfig({
          visible: true,
          title: errMsg.includes('limit') ? 'Limit Reached' : 'Invalid Audio Detected',
          message: err.message || 'Unable to analyze: respiratory sound not detected. Please upload a clear recording of chest/mouth breathing.',
          type: 'warning',
        });
      }, 600);
    }
      ...
      <ScrollView ref={scrollRef} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {!done ? (
          ...
          <Reanimated.View entering={FadeInDown.duration(400)} style={styles.resultsContainer}>
            ...
            {/* 🎧 Listen to Analyzed Audio Track */}
            <AudioPlaybackCard
              audioUri={file?.uri}
              fileName={file?.name || 'respiratory_audio.wav'}
              durationSeconds={time > 0 ? time : 5}
              isWheeze={analysisResult?.wheezingDetected === 'Yes'}
              title="Listen to Analyzed Audio"
            />
